...
col_list = st.beta_columns(len(user_options))  # dplit the screen into columns
...
for idx, (source, options) in enumerate(user_options.items()):
    with col_list[idx]:
        ...
        for option in options:
            ...
            if selected:
                selected_options[source].append(option)

# display selected options
st.write("Selected Options:")
for source, options in selected_options.items():
    for option in options:
        st.write(f'You selected {option} from {source.capitalize()}')